...
exchanges = [ccxt.binance(), ccxt.kucoin(), ccxt.bittrex()]
...
def trading_strategy(exchange, symbol):
    ...
    if current_price < buy_threshold:
        ...
        print('Bought', btc_to_buy, 'BTC at', current_price, 'on', exchange.name)

    # Check if the current price is above the sell threshold
    if current_price > sell_threshold:
        # Place a sell order
        exchange.create_market_sell_order(symbol, btc_balance)
        print('Sold', btc_balance, 'BTC at', current_price, 'on', exchange.name)

# Run the trading strategy on all exchanges
